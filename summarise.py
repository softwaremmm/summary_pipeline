...
​
    for entry in mapping_blob:
        new_species = {}
​
        genome_name = get_field("genome_name", entry, fail_hard).replace(" complete genome", "")
...
​
    ...
    if not (len(phylo_group.keys()) == 1):
        logging.error("Require only 1 phylo group. Found " + str(len(phylo_group.keys())))
        should_fail_hard(fail_hard)
    else:
        myco["Phylogenic Group"]["Name"] = list(phylo_group.keys())[0]
        myco["Phylogenic Group"]["Coverage"] = get_field("percent_coverage", phylo_group[myco["Phylogenic Group"]["Name"]], fail_hard)
        myco["Phylogenic Group"]["Median Depth"] = get_field("median_depth", phylo_group[myco["Phylogenic Group"]["Name"]], fail_hard)
​
    subspecies = get_field("species", mykrobe_blob, fail_hard)
    if not (len(subspecies.keys()) == 1):
        logging.error("Require only 1 species group. Found " + str(len(subspecies.keys())))
        should_fail_hard(fail_hard)
    else:
        myco["Subspecies"]["Name"] = list(subspecies.keys())[0]
        myco["Subspecies"]["Coverage"] = get_field("percent_coverage", subspecies[myco["Subspecies"]["Name"]], fail_hard)
        myco["Subspecies"]["Median Depth"] = get_field("median_depth", subspecies[myco["Subspecies"]["Name"]], fail_hard)
​
​
...
def generate_organism_identification(gatekeeper_blob, fail_hard):
    organism = {"Human Reads": None, # comes from CLI data
                "Unclassified Reads": get_field("Unclassified", gatekeeper_blob, fail_hard),
                "Non-Mycobacterium Bacteria Reads": None,
                "Mycobacterium Reads": get_field("Mycobacteriaceae", gatekeeper_blob, fail_hard)
                }
...
​
    ...
    if bac and organism["Mycobacterium Reads"]:
        organism["Non-Mycobacterium Bacteria Reads"] = bac - organism["Mycobacterium Reads"]
​
    return organism
​
...
​
    ...
    for drug in effects.keys():
        ...
        for mutant in effects[drug]:
            mutant_data = get_field("prediction", mutant, False) # this is horrible as it will throw errors for pheno data
...
​
            ...
            if new_mutation in all_mutations_details.keys():
                position = get_field("gene_position", all_mutations_details[new_mutation], fail_hard)
...
​
            if gene in all_gene_name_pos_res.keys():
                if position in all_gene_name_pos_res[gene].keys():
                    ...
                    logging.error("position not in gene position list: " + gene + " " + str(position))
                    should_fail_hard(fail_hard)
            else:
...
if __name__=="__main__":
    parser = argparse.ArgumentParser(description='Process pipeline output to create mega.json')
    parser.add_argument('--fail_hard', dest='fail_hard', action='store_true', default=False)
    parser.add_argument('--gatekeeper', dest='gatekeeper', help='Path to gatekeeper_report.json file')
    parser.add_argument('--mapping', dest='mapping', help='Path to competitivemapping_report.json file')
    parser.add_argument('--mykrobe', dest='mykrobe', help='Path to mykrobe_report.json file')
    parser.add_argument('--gnominicus', dest='gnom', help='Path to gnomonicus.json file')
​
...
​
    output["Organism Identification"] = generate_organism_identification(gatekeeper_json, args.fail_hard)
    output["Mycobacterium Results"] = generate_mycobacterium_results(mapping_json, mykrobe_json, args.fail_hard)
    output["Sequencing Quality"] = generate_sequencing_quality(mapping_json, args.fail_hard)
    output["Resistance Prediction"] = generate_resistance_prediction(gnom_json, args.fail_hard)
​
